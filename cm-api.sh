...
case "$2" in
        ...
        -a)
                ...
                if ! [[ -z $output ]]; then
                        ...
                        model=$(echo $output | awk -F"," '{print $22}' | awk '{print $NF}')
                        gimur=$(echo $output | awk -F"," '{print $8}' | awk '{print $NF}')
                        testu=$(echo $output | awk -F"," '{print $14}' | awk '{print $NF}')
                        output2=$(curl -s "$url_Degem&q='$degem'" | jq ".result.records[]")
                        echo $output2
                        echo $year, $degem, $model, $gimur, $testu
                        if ! [[ -z "$output2" ]]; then
                                ...
                                echo "Error: there is no results for the query 1"
                        fi
                else
                        echo "Error: there is no results for the query"
                fi
